...
class OverlayManager(private val context: Context) {
    ...
    fun updateMetrics(activeTimeMs: Long, reelCount: Int, isDemoMode: Boolean) {
        val effectiveTimeMs = if (isDemoMode) activeTimeMs * 60 else activeTimeMs
        overlayView?.updateState(effectiveTimeMs, reelCount)
    }
    ...
    private inner class PillOverlayView(context: Context) : View(context) {
        ...
        fun updateState(timeMs: Long, reels: Int) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            val reelString = "$reelCount reels"

            canvas.drawRoundRect(cx - rx, cy - ry, cx + rx, cy + ry, ry, ry, paintPill)
            canvas.drawText(timeString, cx, cy + 5f, paintText)
            canvas.drawText(reelString, cx, cy + 38f, paintSubText)
        }
    }
}
